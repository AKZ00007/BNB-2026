...
export function ConfigPreviewStep({ config: initialConfig, onSave, onBack }: ConfigPreviewStepProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-[#050505] text-slate-900 dark:text-gray-100 font-sans animate-fade-in relative z-10 w-full overflow-hidden transition-colors">

            {/* Top Header Bar */}
            <header className="flex items-center justify-between h-14 border-b border-slate-200 dark:border-[#1F2937]/50 bg-white dark:bg-[#0A0A0A] px-4 flex-shrink-0 z-20 transition-colors">
                <div className="flex items-center gap-2">
                    <button onClick={onBack} className="text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-white transition-colors p-1.5 rounded-lg dark:hover:bg-white/5" title="Start Over">
                        <ArrowLeft className="w-4 h-4" />
                    </button>
                    <div className="h-4 w-px bg-slate-200 dark:bg-white/10 mx-1 hidden sm:block transition-colors" />
                    {/* View Toggles */}
                    <div className="hidden sm:flex bg-slate-100 dark:bg-[#111111] border border-slate-200 dark:border-white/5 rounded-lg p-0.5 transition-colors">
                        <button className="px-3 py-1 text-xs font-medium text-slate-900 bg-white dark:text-white dark:bg-[#1a1a1a] rounded-md shadow-sm transition-colors">Preview</button>
                        <button className="px-3 py-1 text-xs font-medium text-slate-500 hover:text-slate-900 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">Code</button>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    <button
                        ...
                        className="lg:hidden px-3 py-1.5 text-[11px] font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 dark:text-gray-300 dark:bg-white/5 dark:hover:bg-white/10 rounded-lg transition-colors border border-slate-200 dark:border-white/10"
                    >
                        Modify
                    </button>
                    <button className="hidden sm:flex px-3 py-1.5 text-[13px] font-medium text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-gray-400 dark:hover:text-white items-center gap-1.5 rounded-lg dark:hover:bg-white/5 transition-colors">
                        <History className="w-4 h-4" /> History
                    </button>
                    <div className="hidden sm:block h-4 w-px bg-slate-200 dark:bg-white/10 mx-1 transition-colors" />
                    ...
                    <button
                        ...
                        className="px-4 py-1.5 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 dark:text-black dark:bg-white dark:hover:bg-gray-200 rounded-lg transition-colors flex items-center gap-1.5 disabled:opacity-50"
                    >
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 lg:p-10 relative">
                    ...
                    <div className="max-w-[1000px] mx-auto bg-white dark:bg-[#0A0A0A] border border-slate-200 dark:border-white/5 shadow-2xl rounded-2xl p-6 lg:p-8 relative ring-1 ring-slate-200 dark:ring-white/5 transition-colors">
                        ...
                        <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-white/5 rounded-2xl p-6 mb-6 shadow-sm dark:shadow-none transition-colors">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-[15px] font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                                    <Lock strokeWidth={1.5} className="w-4 h-4 text-slate-400 dark:text-gray-400" /> Token Distribution & Vesting
                                </h3>
                                <button className="text-[10px] text-pink-600 dark:text-[#22D3EE] hover:underline font-bold tracking-widest uppercase ml-2 text-right">
                                    View Schedule
                                </button>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {/* PLU Mechanics */}
                            <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-white/5 rounded-2xl p-6 relative overflow-hidden group shadow-sm dark:shadow-none transition-colors">
                                ...
                                <h3 className="text-[15px] font-semibold text-slate-900 dark:text-white flex items-center gap-2 mb-4">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-end border-b border-slate-200 dark:border-white/5 pb-3">
                                        <div>
                                            <p className="text-[10px] text-slate-500 dark:text-gray-500 uppercase tracking-widest font-semibold">Initial Setup</p>
                                            <p className="text-[13px] text-slate-600 dark:text-gray-300 mt-1">LP Lock / Yield</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[13px] font-mono text-slate-900 dark:text-white">{config.plu.totalLockPercent}% LP Lock</p>
                                            <p className="text-[11px] font-mono text-slate-500 dark:text-gray-500">Total</p>
                                        </div>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-slate-500 dark:text-gray-500 uppercase tracking-widest font-semibold mb-2">Unlock Milestones</p>
                                        <ul className="space-y-2">
                                            {config.plu.milestones.map((m, i) => (
                                                <li key={i} className="flex justify-between items-center text-[13px] bg-slate-50 dark:bg-[#0A0A0A] p-2.5 rounded-lg border border-slate-200 dark:border-white/5">
                                                    <span className="text-slate-600 dark:text-gray-400 truncate max-w-[65%]">{m.condition}</span>
                                                    <span className="text-blue-500 dark:text-blue-400 font-mono whitespace-nowrap">Unlock {m.percent}%</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                            ...
                            <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm dark:shadow-none transition-colors">
                                <h3 className="text-[15px] font-semibold text-slate-900 dark:text-white flex items-center gap-2 mb-4">
                                ...
                                <ul className="space-y-3 shrink-0 relative z-10">
                                    <li className="flex items-center gap-3">
                                        ...
                                        <span className="text-[13px] text-slate-600 dark:text-gray-300">Renounce Ownership (Optional)</span>
                                    </li>
                                    <li className="flex items-center gap-3">
                                        <div className="w-5 h-5 rounded-full bg-[#10B981]/20 flex items-center justify-center shrink-0">
                                            <CheckIcon className="w-3 h-3 text-[#10B981]" />
                                        </div>
                                        <span className="text-[13px] text-slate-600 dark:text-gray-300">Liquidity Automatically Locked</span>
                                    </li>
                                    <li className="flex items-center gap-3">
                                ...
                                <div className="mt-6 pt-5 flex flex-wrap justify-between items-center bg-slate-50 dark:bg-[#0A0A0A] p-4 rounded-xl border border-slate-200 dark:border-white/5 relative z-10 shrink-0 transition-colors">
                                    <div>
                                        <p className="text-[10px] text-slate-500 dark:text-gray-500 uppercase tracking-widest font-semibold">Trading Taxes</p>
                                        <div className="flex gap-4 mt-1.5 flex-wrap">
                                            <span className="text-[13px] font-mono text-red-500 dark:text-red-400">Buy: {config.amm.buyTaxPercent}%</span>
                                            <span className="text-[13px] font-mono text-green-500 dark:text-green-400">Sell: {config.amm.sellTaxPercent}%</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
