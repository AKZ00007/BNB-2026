...
import {
    ChevronDown, Zap, Shield, Sparkles, BarChart, Rocket, LayoutGrid,
    LineChart, FileText, Radio, ShieldCheck, LogOut, Menu, X
} from 'lucide-react'
import { ConnectWallet } from '@/components/wallet/connect-button'
import { AnimatedThemeToggler } from '@/components/ui/animated-theme-toggler'
import { useAuth } from '@/components/providers/auth-provider'

/* ─── Mobile nav link data ─────────────────────────────────────────────── */
const mobileFeatures = [
    { href: '/create', label: 'Token Designer', icon: Zap },
    { href: '/templates', label: 'Token Templates', icon: LayoutGrid },
    { href: '/amm', label: 'AMM & Liquidity', icon: BarChart },
    { href: '/plu', label: 'Token Vesting (PLU)', icon: Shield },
];

const mobileSolutions = [
    { href: '/launchpad', label: 'GROWUP AI', icon: Rocket },
    { href: '/growth', label: 'Post-Launch Growth', icon: LineChart },
    { href: '/dashboard', label: 'My Config Dashboard', icon: FileText },
    { href: '/scanner', label: 'Risk Scanner', icon: ShieldCheck },
        ...
        <>
            <div className="fixed top-0 left-0 right-0 z-[100] bg-white dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800 transition-colors duration-300">
                <div className="max-w-[1200px] mx-auto px-4 sm:px-8 h-16 flex items-center justify-between">
                    ...
                    {/* Desktop Radix Navigation */}
                    <NavigationMenu.Root className="relative justify-center z-[1] hidden md:flex">
            ...
            {mobileOpen && (
                <div className="fixed inset-0 z-[99] md:hidden">
                    ...
                    {/* Slide-in panel */}
                    <div className="absolute top-16 left-0 right-0 bottom-0 bg-white dark:bg-gray-950 overflow-y-auto animate-in slide-in-from-top-2 duration-200">
                        <div className="p-6 space-y-6">
                                ...
                                {user ? (
                                    <>
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-primary to-purple-500 flex items-center justify-center text-xs font-bold text-white">
                                        ...
                                        <button
                                            ...
                                            className="flex items-center gap-2 w-full px-4 py-2.5 rounded-lg border border-red-200 dark:border-red-800/50 text-red-600 dark:text-red-400 text-sm font-medium hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors"
                                        >
                                    ...
                                    <div className="flex gap-3">
                                        <Link
                                            href="/login"
                                            onClick={() => setMobileOpen(false)}
                                            className="flex-1 text-center px-4 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
                                        >
                                            Login
                                        </Link>
                                        <Link
                                            href="/signup"
                                            onClick={() => setMobileOpen(false)}
                                            className="flex-1 text-center px-4 py-2.5 rounded-lg bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors"
                                        >
                                            Sign Up
                                        </Link>
                                    </div>
                            ...
                            {/* Features */}
                            <div>
                                <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Features</h3>
                                <div className="space-y-1">
                                    {mobileFeatures.map(({ href, label, icon: Icon }) => (
                                        <Link
                                            key={href}
                                            href={href}
                                            onClick={() => setMobileOpen(false)}
                                            className="flex items-center gap-3 px-3 py-3 rounded-lg text-[15px] font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
                                        >
                            ...
                            {/* Solutions */}
                            <div>
                                <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Solutions</h3>
                                <div className="space-y-1">
                                    {mobileSolutions.map(({ href, label, icon: Icon }) => (
                                        <Link
                                            key={href}
                                            href={href}
                                            onClick={() => setMobileOpen(false)}
